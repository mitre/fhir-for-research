...
-   [**RxNorm**](https://www.nlm.nih.gov/research/umls/rxnorm/index.html): a "normalized naming system for generic and branded drugs" [@RxNorm_Overview]

-   [**CVX**](https://www2a.cdc.gov/vaccines/iis/iisstandards/vaccines.asp?rpt=cvx): "Vaccines Administered" code system

Because these code systems are so commonly part of medical data, a more detailed description of each is included below. Additionally, [HIMSS provides a longer list of code systems](https://www.himss.org/resources/terminology-standards) that may appear in medical data.
...
SNOMED CT [@snomed] is comprised of:
...
[ICD](https://www.who.int/standards/classifications/classification-of-diseases) is a medical classification list that contains codes for diseases, signs and symptoms, abnormal findings, complaints, social circumstances, and external causes of injury or diseases. The [World Health Organization](https://www.who.int) (WHO), which publishes ICD, [writes about classification](https://www.who.int/standards/classifications/classification-of-diseases):
...
ICD-10 is widely used and replaced ICD-9 in the US in the 2010s. [ICD-11](https://icd.who.int/en/) is the latest version of ICD, with active use beginning in January 2022 internationally. As of 2026, ICD-11 is not actively used by EHRs in the US.
...
Below is an example of different levels of specificity for an ICD-10 code [@icd_10]. The injury is a *closed fracture of distal phalanx of right index finger*. **Bolded** text below indicates a traversal of least to most specific codes for this injury.
...
NLM completes the following three steps when classifying a drug [@RxNorm_Overview]:
...
-   [**ConceptMap resource**](https://www.hl7.org/fhir/conceptmap.html): Identifies relationships between sets of concepts. E.g., [Mapping from SNOMED CT to ICD-10](https://www.hl7.org/fhir/conceptmap-103.html)
...
-   Properties of the code system, such as concept permanence, case-sensitivity, and if it defines a [compositional grammar](https://www.hl7.org/fhir/codesystem-definitions.html#CodeSystem.compositional).

Here's [an example, fictitious CodeSystem FHIR resource](https://fhir.hl7.org/fhir/codesystem-example.html) from the FHIR specification:
...
Note that all of the common code systems listed above (LOINC, SNOMED-CT, etc.) are listed as ["external" code systems in the FHIR specification](https://hl7.org/fhir/terminologies-systems.html). This means that their codes and descriptions are not published in FHIR format inside the FHIR specification, and the contents of these code systems can change over time -- sometimes frequently -- as codes are modified to meet real-world implementation needs. In contrast, internal code systems like [Medication Status](https://hl7.org/fhir/codesystem-medication-status.html) are published in [FHIR format](https://hl7.org/fhir/codesystem-medication-status.json.html) and are only changed when the FHIR specification is updated.

The easiest way to get external code systems in FHIR format is to query a [terminology service](https://fhir.hl7.org/fhir/terminology-service.html) like `tx.fhir.org`. (Note that this terminology service [is maintained by the FHIR community](https://confluence.hl7.org/spaces/FHIR/pages/79503413/tx.fhir.org+Documentation) and is not guaranteed to be perfectly in sync with the canonical list of codes for a given code system. If you notice issues, you can ask for help on [chat.fhir.org](https://chat.fhir.org); see more in the [FHIR Community & Documentation](/modules/fhir-community.qmd) module.)

You can query `tx.fhir.org` or another terminology service if you know the code system's URI. For example, [CVX is identified by `http://hl7.org/fhir/sid/cvx`](https://terminology.hl7.org/CVX.html), so its contents can be requested from `tx.fhir.org` by visiting <https://tx.fhir.org/r4/CodeSystem?url=http://hl7.org/fhir/sid/cvx>.
...
Here's an example of a ValueSet FHIR resource for coding [how tumor size is measured in the oncology domain](https://www.hl7.org/fhir/us/mcode/ValueSet-mcode-tumor-size-method-vs.html):
