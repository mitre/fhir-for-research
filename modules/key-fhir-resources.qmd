...
learning_objectives:
  - question: What are three key FHIR resources?
    answer: Patient, Observation, and Bundle. 
  - question: How are FHIR resources defined?
  ...
  - question: How do FHIR resources support interoperability?
    answer: A FHIR resource's attributes are rigorously defined, which allows systems to transfer medical information without loss of meaning.
---
...
The FHIR core v4.3.0 (also known as Revision 4B, or R4B) defines [140 resources](https://hl7.org/fhir/R4B/resourcelist.html) that allow for healthcare interoperability. The [HL7 FHIR specification](https://hl7.org/fhir) gives a well-structured summary of FHIR's core resources:

[![The organization of FHIR's core resources. From [HL7's FHIR specification](https://hl7.org/fhir/R4B).](images/fhir-index.png){fig-alt="Screenshot from the FHIR specification showing the organization of FHIR resources. Level 1: basic framework on which the specification is built. Level 2: Supporting implementation and binding to external specifications. Level 3: Linking to real world concepts in the healthcare system. Level 4: Record-keeping and and Data Exchange for the healthcare process. Level 5: Providing the ability to reason about the healthcare process."}](https://hl7.org/fhir/R4B)
...
## The Patient resource: An introduction to the FHIR resource structure
...
[![The structure for FHIR Patient resource.](images/fhir-r4b-patient-structure.png){.column-page-inset fig-alt="The structure for FHIR Patient resource. Go to https://hl7.org/fhir/R4B/patient.html#resource for a text version."}](https://hl7.org/fhir/R4B/patient.html#resource)

All FHIR resources have a similar table representing their contents. These tables have the following columns:

-   **Name:** The element name.
-   **Flags:** Special markers that carry information required for implementers. The flags **`N`**, `?!`, and `Σ` represent [normative](https://www.hl7.org/fhir/R4B/versions.html#std-process), [is-modifier](https://www.hl7.org/fhir/R4B/conformance-rules.html#isModifier), and [in-summary](https://www.hl7.org/fhir/R4B/search.html#summary) respectively. We will not cover FHIR attribute flags in detail here, but [HL7 documents them](https://www.hl7.org/fhir/formats.html#table).
...
-   **Type:** The primitive or complex [data type](https://hl7.org/fhir/datatypes.html). A [primitive data type](https://www.hl7.org/fhir/datatypes.html#primitive) is a base data type, like [`date`](https://hl7.org/fhir/R4B/datatypes.html#date) or [`integer`](https://hl7.org/fhir/R4B/datatypes.html#integer). A [complex data type](https://hl7.org/fhir/R4B/datatypes.html#complex) is constructed from other data types, like [Address](https://hl7.org/fhir/R4B/datatypes.html#Address).
...
`Patient.name` is of complex type [HumanName](https://hl7.org/fhir/datatypes.html#HumanName). Complex types are FHIR resources and have their own structure table. Here's the one for HumanName: <!-- FIX Whatever you decide "Patient" should be above (with or without ``) should be applied to HumanName --LDC -->

[![The structure for Patient.name, which is a HumanName.](images/fhir-human-name-structure.png){.column-page-inset fig-alt="The structure for Patient.name, which is a HumanName. Go to https://hl7.org/fhir/datatypes.html#HumanName for a text version."}](https://hl7.org/fhir/datatypes.html#HumanName)
...
This degree of flexibility is not helpful in all use cases. FHIR addresses this through [resource Profiles](https://hl7.org/fhir/profiling.html), which provide a way to computably describe use case-specific constraints (e.g., "all names must include a `family` element"). Profiles are collected in [Implementation Guides (IGs)](/modules/30-fhir-data-modeling--real-world-ig/) to provide a complete specification based on FHIR for a given use case.
:::
...
![The first few rows of the structure table of the Patient resource, with an arrow indicating that Patient is a DomainResource.](images/fhir-r4b-patient-domainresource.png){.column-page-inset fig-alt="The first few rows of the structure table of the Patient resource, with an arrow indicating that Patient is a DomainResource."}
...
[![The structure for FHIR observation resource.](images/fhir-observation-structure.png){.column-page-inset fig-alt="Observation structure with attributes status:code, code:CodeableConcept, subject:Reference(Patient), and value[x]. Go to https://hl7.org/fhir/observation.html#resource for a text version."}](https://hl7.org/fhir/observation.html#resource)
...
-   **`Observation.id = f001`**: The `id` attribute from the grandparent class `Resource`. See [accessing FHIR data](/modules/10-intro--accessing-fhir-data/) for how to use a resource id.
...
-   **`CapabilityStatement.implementationGuide`:** The [implementation guides (IG)](/modules/30-fhir-data-modeling--real-world-ig/) that the FHIR server conforms to. However, a server may be partially or fully compliant to an IG without listing it in the metadata.
-   **`CapabilityStatement.rest[i].security.service`:** The [RESTful security services](http://hl7.org/fhir/valueset-restful-security-service.html#expansion) required to access data on this server. This example uses [SMART-on-FHIR](/modules/20-intro-smart-on-fhir--overview/), but other possibilities include [OAuth](https://oauth.net/) or [UDAP](https://www.udap.org/).
...
:   A problem, diagnosis, or clinical event. It should be the reason for a medical intervention and tie into a larger [clinical workflow](http://hl7.org/fhir/workflow.html). There is some use-case overlap with `Observation`; an [IG](/modules/30-fhir-data-modeling--real-world-ig/) will provide better guidance.
