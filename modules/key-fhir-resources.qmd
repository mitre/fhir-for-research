...
The [Fire Maturity Model (FMM)](https://hl7.org/fhir/versions.html#maturity) describes how mature the definition of a particular FHIR resource type is. Each resource types has a maturity level from FMM 0 (draft) to FMM 6 (normative). As resource types receive more testing and become more stable, their FMM level increases, and in general, resource types with higher FMM levels are more likely to be implemented in real world systems. The FMM level for each resource type is displayed to the right of its name in the resource list, with `N` indicating normative.
