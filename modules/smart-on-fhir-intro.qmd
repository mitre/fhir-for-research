...
aliases:
  - new-smart-on-fhir-intro.html
---

Health information systems such as Electronic Health Record (EHR) systems contain data useful to a wide variety of healthcare applications, but they often lack the support for developing sophisticated applications which can use that data. Third-party application developers need a solution which allows healthcare applications to securely integrate and exchange data with these systems. [SMART on FHIR](https://smarthealthit.org)[^1] was introduced in 2016 "to enable medical applications to be written once and run unmodified across different healthcare IT systems." [@Mandel_Kreda_Mandl_Kohane_Ramoni_2016]
...
The primary goal of SMART on FHIR was to provide a common interface that allowed applications to connect seamlessly and securely to a FHIR API, ensuring applications could provide access only to authorized users. Applications can be developed for a variety of platforms (web, desktop, mobile) and use the same secure method to connect to a variety of EHR systems.
...
Other common uses for SMART on FHIR applications include gathering [Questionnaire respsonses](https://hl7.org/fhir/questionnaireresponse.html). These are frequently used to gather information from patients such as family medical history, mental health, diet and nutrition and social determinants of health.
...
SMART on FHIR interacts with the EHR's FHIR server API using standard HTTP methods. This means that data analytics applications created using common data analysis tools for R or Python can also integrate SMART on FHIR connectivity, allowing health data from an EHR to be accessed directly.
...
-   The [CDS Hooks module on this website](new-cds-hooks-intro.qmd).
...
Consider security and privacy early when developing your research approach. Take steps to ensure proper data storage and access as you would with any software that handles sensitive data. Patient data should not be stored or exchanged with additional services outside of the program or application, such cloud-based computing, storage or AI services unless explicitly authorized by your organization, and accredited to handle sensitive data.
...
While SMART on FHIR provides the functionality to unlock and exchange data with an EHR, you must consider that this access comes at a performance cost. FHIR-formatted patient data can be computationally expensive to generate, and FHIR patient bundles can easily contain thousands of resources, resulting in hundreds of megabytes of data if you are not selective about which patient resources are required for your purpose. Your application should make requests for only the necessary data required for your functionality, and be careful to limit the breadth and depth of requests, whether made intentionally or not.

In addition, some EHR vendors provide FHIR APIs via a subscription or tiered-licensing fee to access data directly from an EHR instance, particularly for high-volume or rate-limited services. If you are working on developing a SMART on FHIR application which is intended to connect to a production ('live') EHR instance, be sure to discuss potential costs with your EHR vendor support services.
