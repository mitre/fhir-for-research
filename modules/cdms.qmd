...
-   **FHIR:** FHIR defines [resources](key-fhir-resources.qmd) for each type of data, which can be extended to add additional data elements as needed for a given use case. Resources have a nested structure (like a multi-level outline) rather than a tabular structure (like a spreadsheet). The base FHIR specification does not have strict terminology constraints, though these can be added for a specific use case in an [Implementation Guide](data-modeling-reading-igs.qmd).
...
-   **Mapping between CDMs:** For example, the [Common Data Models Harmonization IG](http://hl7.org/fhir/us/cdmh/) maps between US Core and PCORNet using FHIR. This same approach can be used to map FHIR data elements to CDEs or other data models.
