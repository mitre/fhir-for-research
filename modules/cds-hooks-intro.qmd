...
CDS Hooks are a technology that is part of the HL7 [FHIR](fhir-from-10000-ft.qmd) ecosystem, and are designed to be easily used with FHIR and [SMART on FHIR](smart-on-fhir-intro.qmd).
...
3.  **Response format**: CDS services return [JSON cards](https://cds-hooks.hl7.org/#cds-service-response), which can reference FHIR-formatted data as part of their responses.
...
An example of a research-focused application leveraging CDS Hooks can be found here: [Automated Clinical Trial Cohort Definition and Evaluation with CQL and CDS-Hooks](https://doi.org/10.3233/shti240028).
...
-   **Definition**: The idea behind CDS Hooks is to "hook into" specific moments in the clinician's workflow (called **hooks**) and trigger decision support services that can provide actionable insights, recommendations, or alerts. Hooks can be triggered when a user opens a patient's electronic health record (`patient-view` hook), when an order is selected (`order-select` hook), when an order is signed (`order-sign` hook), when a patient encounter or visit begins (`encounter-start` hook), etc.

-   **Further information**: A more detailed overview on hooks and examples of hook types can be found at the section on [Types of CDS Hooks](cds-hooks-types.qmd).
...
-   **Interoperability and Standards-Based**: Built on open standards like RESTful APIs and FHIR, CDS Hooks allow for broad compatibility across different EHR systems, promoting interoperability and ease of implementation in diverse healthcare settings.
...
While the CDS Hooks framework provides considerable capabilities to enhance clinical decision support in applications, poor design can lead to alert fatigue and other drawbacks that may detract from the intended impacts of an application. Therefore, following best practices in user-centered design is important when using CDS Hooks. Key considerations include:
