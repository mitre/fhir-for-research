...
learning_objectives:
  - question: Summarize the role of FHIR in health data interoperability.
    answer: FHIR is a common specification that defines technical mechanisms for interoperability of health IT systems. Adoption and implementation of FHIR is driven by regulation in the US, and facilitated by FHIR's pragmatic implementation-first approach.
  - question: Explain FHIR’s relevance to research.
    answer: At a high level, FHIR is well-adopted throughout the US healthcare system thanks to ONC and CMS rules. FHIR's openness, active development with an implementer-first approach, and extensibility make it a flexible tool for many use cases including research-focused data access and exchange.
  - question: Describe how FHIR can enhance research.
    answer: FHIR is useful for research primarily through providing direct access to health data through a standardized API that would otherwise have to be accessed with one-off analyst work or through a more complex workflow. Using FHIR alongside related technologies also provides other benefits to research; these are discussed further in other modules on this website.
...
[^2]: ONC is [The Office of the National Coordinator for Health Information Technology](https://www.healthit.gov/).
    ...
    From [their website](https://healthit.gov/about/):

    > ASTP/ONC is the principal federal entity charged with coordination of nationwide efforts to implement and use the most advanced health information technology and the electronic exchange of health information.
...
These FHIR resources are [designed to include data elements that cover the majority of use cases](http://www.healthintersections.com.au/2014/02/28/fhir-and-confusion-about-the-8020-rule.html), and the FHIR specification provides [mechanisms](https://www.hl7.org/fhir/extensibility.html) for extending data models when necessary.
